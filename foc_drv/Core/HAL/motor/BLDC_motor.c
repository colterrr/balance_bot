...
void foc_test(BLDC_motor* obj)
{
    obj->motor_foc->angle_dq = obj->motor_encoder->angle_rad * obj->config.pole_pair; //更新dq系电角度 
    // vector_ab ab;
    if (obj->EN == 0) {
    ...
    obj->motor_foc->angle_dq = obj->motor_encoder->angle_rad * obj->config.pole_pair; //更新dq系电角度 
        vector_ab ab;
        ab.alpha = 0.1*cos(3.14159 *2 / 40 * i);
        ab.beta = 0.1*sin(3.14159 *2 / 40 * i);
...
void BLDC_motor_calc(BLDC_motor* obj)
{
    ...
    if (obj->config.foc_cfg.foc_type == CURRENT_CLOSE){
        ...
        //此项过后output表示与最大输出的比例（在-sqrt_3/2和sqrt_3/2之间）
        obj->output = bound(-SQRT_3 /2, obj->output * obj->config.R_phase / obj->config.torque_cons / obj->config.voltage_max, SQRT_3 /2);
    }
...
void BLDC_motor_Update()
{
    for (uint8_t i = 0; i < BLDC_motor_ins->cv_len; i++){
        ...
        if (!obj->motor_encoder->init_flag) {//编码器没有校准，退出
            obj->EN = 0;
            return; 
        }
        if (!obj->init_flag){
        ...
        if (!BSP_GPIO_in(obj->config.fault_port)) { //驱动芯片报错，退出
            obj->ERR = 1;
            obj->EN = 0;
        }
        if (obj->commu_sta){ //与主机失去通信，退出
            //obj->EN = 0;
        }        
        BLDC_motor_calc(obj);            
        //foc_test(obj);
