...
#define GRAVITY 9.78f //深圳市重力加速度,单位m/s²
...
void MahonyAHRS_update(MahonyAHRS *obj, float gx, float gy, float gz, float ax,
    ...
    static float maxDifference = 0.33; // 超过该值则不跟随
    static float differenceThreshold = 0.1;
    static float gyroSumThreshold = 0.12;
    ...
    if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
        ...
        difference = fabsf(accNorm-GRAVITY);
        ax *= recipNorm;
        ay *= recipNorm;
        az *= recipNorm;
        ...
        if(difference > maxDifference)
            obj->scale=0;
        else if(difference > differenceThreshold && gyroSum > gyroSumThreshold)
            obj->scale = differenceThreshold / difference * gyroSumThreshold / gyroSum;
        else if(difference > differenceThreshold)
            obj->scale = differenceThreshold/difference;
        else if(gyroSum > gyroSumThreshold)
            obj->scale = gyroSumThreshold / gyroSum;
        else
            obj->scale = 1;
        ...
        gx += obj->twoKp * halfex * obj->scale;
        gy += obj->twoKp * halfey * obj->scale;
        gz += obj->twoKp * halfez * obj->scale;
    }
